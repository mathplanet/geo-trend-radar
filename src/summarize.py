"""GEO Trend Radar - 2단계: 요약 -> 클러스터링 -> 헤드라인 선정

BUILD-GUIDE.md 2단계 / REQUIREMENTS.md FR-6 구현.
3단계 전환: 입력/출력 모두 Supabase (items, digests 테이블).

모델 전략 (REQUIREMENTS.md D-3):
- 건별 요약/관련도 재평가: 저가 모델(Haiku)
- 헤드라인 선정/종합 총평: 상위 모델
언어 (D-1): 원제목은 영문 유지, 요약/시사점은 한글.
"""
import argparse
import json
import os
import sys
from datetime import datetime, timezone

import anthropic

from store import (
    fetch_all_items,
    update_item_cluster,
    update_item_summary,
    upsert_digest,
    within_window,
)

PER_ITEM_MODEL = "claude-haiku-4-5-20251001"
DIGEST_MODEL = "claude-sonnet-5"

PER_ITEM_SYSTEM = """당신은 GEO/SEO 컨설팅 팀을 위한 리서치 어시스턴트입니다.
주어진 글의 제목과 원문 요약만 보고 아래 JSON 형식으로만 답하세요. 다른 텍스트는 출력하지 마세요.

{"relevant": true|false, "summary": "핵심 사실 3개 (줄바꿈으로 구분, 한글)", "cluster": "주제 분류 (한글, 1~3단어)", "insight": "컨설턴트가 클라이언트에게 전달할 실무 시사점 (한글, 1~2문장)"}

- relevant: 이 글이 GEO(생성형 검색 최적화)/SEO 트렌드와 실질적으로 관련 있으면 true, 단순 홍보/채용/이벤트 공지 등 트렌드성이 없으면 false.
- summary: 완전한 문단이 아니라 고객에게 바로 공유할 수 있는 **불릿 3개**. 각 줄은 한 문장, 15단어 이내로 핵심 사실만 담고 수식어·중복 설명은 뺄 것. 원문을 안 읽어도 핵심을 이해할 수 있어야 함.
- insight: 뻔한 요약 반복이 아니라 "그래서 클라이언트에게 뭘 해야 하는가" 관점."""

DIGEST_SYSTEM = """당신은 GEO/SEO 트렌드 주간 다이제스트를 만드는 편집자입니다.
아래는 이번 주 수집된 글 목록(id·주제·요약·시사점)입니다.
가장 중요한 3~5개를 헤드라인으로 선정하고, 아래 JSON 형식으로만 답하세요.

{"headline_ids": [<int>, ...], "overview": "이번 주 핵심 3~5개 (줄바꿈으로 구분, 한글)"}

- overview: 완전한 문단이 아니라 **불릿 3~5개**. 각 줄은 한 문장, 20단어 이내로 이번 주 가장 중요한 사실만 담을 것. 수식어·중복 설명 없이 5분 안에 훑을 수 있어야 함."""

CATEGORY_INSIGHTS_SYSTEM = """당신은 GEO/SEO 트렌드 리포트의 섹션 에디터입니다.
아래는 이번 주 다이제스트를 그룹(대분류)별로 나눈 글 목록입니다 (한 글이 여러 그룹에 속할 수 있음).
각 그룹마다 그 글들을 종합해 핵심 사실 2~3개를 불릿으로 작성하세요 (줄바꿈으로 구분, 한글, 각 줄 20단어 이내).
완전한 문단이 아니라 간결한 사실 나열이어야 합니다. 글이 없는 그룹은 결과에서 제외하세요.

JSON으로만 답하세요: {"category_insights": {"<그룹명>": "불릿 2~3개 (줄바꿈 구분)", ...}}"""

CLUSTER_SYSTEM = """당신은 GEO/SEO 트렌드 리포트의 편집자입니다.
아래는 이번 주 관련 글 전체 목록(id·제목·요약)입니다.
서로 비슷한 주제의 글들을 묶을 수 있는 소제목(클러스터) 5~10개를 정의하고, 모든 글을 그중 하나에 배정하세요.

- 내용이 비슷한 글은 반드시 같은 클러스터로 묶어서, 거의 같은 뜻의 소제목이 여러 개 생기지 않게 하세요.
- 소제목은 한글 2~5단어로 간결하게.

JSON으로만 답하세요: {"assignments": {"<id>": "<클러스터명>", ...}}"""


def extract_text(response):
    """extended thinking 등으로 content[0]이 텍스트가 아닐 수 있어 text 블록을 직접 찾는다."""
    for block in response.content:
        if block.type == "text":
            return block.text
    raise ValueError("응답에 text 블록이 없습니다")


def parse_json_response(response):
    text = extract_text(response).strip()
    if text.startswith("```"):
        text = text.strip("`")
        if text.startswith("json"):
            text = text[4:]
    return json.loads(text)


def summarize_item(client, item):
    user_content = f"제목: {item['title']}\n원문 요약: {item.get('raw_summary') or '(없음)'}"
    resp = client.messages.create(
        model=PER_ITEM_MODEL,
        max_tokens=500,
        system=PER_ITEM_SYSTEM,
        messages=[{"role": "user", "content": user_content}],
    )
    return parse_json_response(resp)


def build_digest(client, items):
    lines = [
        f"- [id={item['id']}] ({item['cluster']}) {item['title']}\n"
        f"  요약: {item['summary']}\n  시사점: {item['insight']}"
        for item in items
    ]
    resp = client.messages.create(
        model=DIGEST_MODEL,
        max_tokens=4000,
        system=DIGEST_SYSTEM,
        messages=[{"role": "user", "content": "\n".join(lines)}],
    )
    return parse_json_response(resp)


def build_category_insights(client, items):
    """그룹(대분류)별 총평 생성. 한 글이 여러 그룹에 속할 수 있어(multi-tag) 그룹별로 중복 포함될 수 있다."""
    by_category = {}
    for item in items:
        for category in item.get("categories") or []:
            by_category.setdefault(category, []).append(item)

    if not by_category:
        return {}

    sections = []
    for category, cat_items in by_category.items():
        lines = [f"- {it['title']} : {it['insight']}" for it in cat_items]
        sections.append(f"[{category}] ({len(cat_items)}건)\n" + "\n".join(lines))

    resp = client.messages.create(
        model=DIGEST_MODEL,
        max_tokens=4000,
        system=CATEGORY_INSIGHTS_SYSTEM,
        messages=[{"role": "user", "content": "\n\n".join(sections)}],
    )
    result = parse_json_response(resp)
    return result.get("category_insights", {})


def build_clusters(client, items):
    """이번 주 관련 글 전체를 한 번에 보고 일관된 소제목(클러스터)으로 재배정한다.
    글마다 독립적으로 생성된 cluster는 비슷한 주제도 미묘하게 다른 라벨을 얻어 과도하게
    쪼개지는 문제가 있었음 (ISSUES.md 참고) - 이 배치 재배정으로 대체한다."""
    lines = [
        f"- [id={item['id']}] {item['title']}\n  요약: {item['summary']}"
        for item in items
    ]
    resp = client.messages.create(
        model=DIGEST_MODEL,
        max_tokens=16000,
        system=CLUSTER_SYSTEM,
        messages=[{"role": "user", "content": "\n".join(lines)}],
    )
    result = parse_json_response(resp)
    return {int(k): v for k, v in result.get("assignments", {}).items()}


def compute_week_label(items):
    """실행 시각(datetime.now())으로 주차를 정하면, 스크립트가 자정 근처에 걸쳐 실행될 때
    실제 데이터의 주차와 라벨이 어긋날 수 있다 (ISSUES.md 참고). collected_at(우리가 기록,
    항상 신뢰 가능) 중 가장 최근 값을 기준으로 주차를 정해 이 문제를 피한다."""
    dates = [item.get("collected_at") or item.get("published_at") for item in items]
    dates = [d for d in dates if d]
    if not dates:
        return datetime.now(timezone.utc).strftime("%G-W%V")
    latest = max(datetime.fromisoformat(d.replace("Z", "+00:00")) for d in dates)
    return latest.strftime("%G-W%V")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--days", type=int, default=7, help="최근 N일 items만 대상 (기본 7)")
    parser.add_argument("--force", action="store_true", help="이미 요약된 글도 다시 요약")
    args = parser.parse_args()

    api_key = os.environ.get("ANTHROPIC_API_KEY")
    if not api_key:
        sys.exit("ANTHROPIC_API_KEY 환경변수가 필요합니다.")
    client = anthropic.Anthropic(api_key=api_key)

    items = fetch_all_items()
    targets = [
        item for item in items
        if within_window(item, args.days) and (args.force or item.get("summary") is None)
    ]
    print(f"요약 대상 {len(targets)}건 (전체 {len(items)}건 중 최근 {args.days}일, force={args.force})")

    for item in targets:
        try:
            result = summarize_item(client, item)
        except Exception as e:
            print(f"[실패] {item['title'][:50]}: {e}")
            continue
        item["relevant"] = result.get("relevant", True)
        item["summary"] = result.get("summary")
        item["cluster"] = result.get("cluster")
        item["insight"] = result.get("insight")
        update_item_summary(item["id"], item["summary"], item["cluster"], item["insight"], item["relevant"])
        mark = "O" if item["relevant"] else "X"
        print(f"[{mark}] ({item['cluster']}) {item['title'][:60]}")

    digest_candidates = [
        item for item in items
        if within_window(item, args.days) and item.get("summary") and item.get("relevant", True)
    ]
    if not digest_candidates:
        print("헤드라인을 선정할 관련 글이 없습니다.")
        return

    cluster_assignments = build_clusters(client, digest_candidates)
    print(f"\n재클러스터링: {len(set(cluster_assignments.values()))}개 소제목으로 정리")
    for item in digest_candidates:
        new_cluster = cluster_assignments.get(item["id"])
        if new_cluster and new_cluster != item.get("cluster"):
            item["cluster"] = new_cluster
            update_item_cluster(item["id"], new_cluster)

    digest = build_digest(client, digest_candidates)
    week_label = compute_week_label(digest_candidates)
    headline_ids = digest.get("headline_ids", [])
    overview = digest.get("overview")
    category_insights = build_category_insights(client, digest_candidates)
    upsert_digest(week_label, headline_ids, overview, category_insights)

    by_id = {item["id"]: item for item in items}
    print(f"\n=== {week_label} 다이제스트 ===")
    print(overview)
    print("\n헤드라인:")
    for hid in headline_ids:
        item = by_id.get(hid)
        if item:
            print(f"- {item['title']} - {item['insight']}")

    print("\n그룹별 총평:")
    for category, insight in category_insights.items():
        print(f"[{category}] {insight}")


if __name__ == "__main__":
    main()
